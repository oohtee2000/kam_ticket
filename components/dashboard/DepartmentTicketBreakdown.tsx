...
export default function DepartmentTicketBreakdown() {
  ...
  useEffect(() => {
    const fetchDepartmentData = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/tickets/metrics/department-breakdown`);
        const data = await res.json();

        setSeries([{ name: "Tickets", data: data.map((item: any) => item.ticketCount) }]);
        setCategories(data.map((item: any) => item.department));
      } catch (err) {
